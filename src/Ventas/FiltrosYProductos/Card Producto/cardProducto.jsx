...
export default function CardProducto(args) {
  ...
  const ofertaEncontrada = ofertas.find(p => p.idProducto == idParaUsar);
        ...
        {state.userInfo &&
          ((state.userInfo.tipo_usuario == 'admin' || state.userInfo.tipo_usuario == 'colaborador') &&
            <>
              ...
              <input type="number" className="inputDescuento" defaultValue={ofertaEncontrada ? ofertaEncontrada.descuento : null} onChange={(e) => setDescuento(e.target.value)} />
              <button className={`ofertarProducto ${ofertaEncontrada && 'active'}`} onClick={() => ((ofertasSelected || descuento <= 0) ? eliminarOferta(idParaUsar) : ofertarProducto(idParaUsar, descuento))}>
